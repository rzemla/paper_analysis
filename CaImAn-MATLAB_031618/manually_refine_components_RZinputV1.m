function [A,C,newcenters,CC] = manually_refine_components_RZinputV1(Y,A,C,centers,img,sx,options,manualCenter)
...
%forces an add ROi operation
button =1;
%how many  ROIs to add for init 
nbROItoAdd = size(manualCenter,1);
disp(manualCenter)
    
for ii=1:nbROItoAdd 
    disp([num2str(ii),'/',num2str(nbROItoAdd)])
...
%         pixel=round([x y]);
        if button==1
            disp(['Adding pixel at:' num2str(manualCenter(ii,1:2))])
            newcenters=[newcenters; manualCenter(ii,:)];
            %remove code above and have it cycle through newcenters with
            %manully selected components; make sure button is set to 1
            %add newcenters to the existing newcenters dummy point
