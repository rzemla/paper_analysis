...
if refine_components
    ...
    [A2,C2,center] = manually_refine_components(data.Y,A,C,cm,Cn,tau,options);
end
...
C_keep = C(keep,:);
figure;
    subplot(121); montage(extract_patch(A(:,keep),[dims(1),dims(2)],[30,30]),'DisplayRange',[0,0.15]);
        title('Kept Components');
    subplot(122); montage(extract_patch(A(:,~keep),[dims(1),dims(2)],[30,30]),'DisplayRange',[0,0.15])
...
plot_components_GUI(data.Yr,A,C,b,f,Cn,options);
