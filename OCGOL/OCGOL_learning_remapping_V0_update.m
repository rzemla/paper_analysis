...
%% Export all lap data (not rate mapped) for model/prediction (place cell recruitment by learning) for Jason
...
%trial type vector (2  = A), (3 = B)
%trial correct vector 1 = correct; 0 - incorrect

%transients (binary) matrix ROI idx x time(frames) - these need to be
%extracted from the match indexes


%create a function to extact this data from each animal and then run
%through each and save as mat file

%time resampled
session_vars{1, 1}.Behavior.resampled.time
%run state
session_vars{1, 1}.Behavior.run_ones  
%position (cm)
p_cm = session_vars{1, 1}.Behavior.resampled.position;
%position norm
p_n = session_vars{1, 1}.Behavior.resampled.normalizedposition;
%lap number
session_vars{1, 1}.Behavior.resampled.lapNb
%speed
session_vars{1, 1}.Behavior.speed  
...
x = session_vars{1, 1}.Behavior.lick.time;
y = session_vars{1, 1}.Behavior.resampled.time;
...
%plot position time and lick
% figure
% hold on
% plot(y,lick_vec)
% plot(y,p -1)

%TRIAL INFORMATION AND CORRECT DATA
%contruct A, B, punish vector 
%check which one of these had the discarded trial information data
...
t_order = session_vars{1}.Behavior.performance.trialOrder;
...
%all else = technical problems on lap and excluded from analysis

%lap number (complete) of each frame
lap_nb = session_vars{1, 1}.Behavior.resampled.lapNb;

%make empty vector for trial type
trial_type_vec = zeroes(





session_vars{1, 1}.Behavior.performance.trialCorrect

session_vars{1, 1}.Behavior.performance.trialOrder  
