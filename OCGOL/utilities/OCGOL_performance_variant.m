function [Behavior] = OCGOL_performance_variant(Behavior,CSV)

%Get performance of the animal on the OCGOL trials

%input combined CSV from all sessions and Behavior file
...
%bin location of rewards
binReward1 = 59;
binReward2 = 139;


%% index, position, and time of licks
%raw CSV lick signal
lickSignal = CSV(:,8);
...
binReward1 = RewardLoc.medALoc;
binReward2 = RewardLoc.medBLoc;

%rewardRange in cm (10 cm)
binRewardRange1 = [binReward1, (binReward1+rewardZoneSize)];
binRewardRange2 = [binReward2, (binReward2+rewardZoneSize)];

%anticipatory range in cm
binAntRange1 = [(binReward1 -1 -rewardZoneSize),(binReward1 -1)];
binAntRange2 = [(binReward2 -1 -rewardZoneSize),(binReward2 -1)];
...
%plot raw normalized location and trial type signal - debugging
%picks up partial lap at the end
%filter out signals that are outside complete laps
%
%    figure;
%    hold on
%    plot(Behavior{1}.normalizedposition,'r')
%    plot(trialSignal,'b');
