function [task_selective_ROIs] = task_selective_categorize(cent_diff_AandB, tuned_logical, pf_vector_max, session_vars,max_transient_peak, options)
...
Aonly_tuned_idx_si = find(tuned_logical.si.onlyA_tuned == 1);
Bonly_tuned_idx_si = find(tuned_logical.si.onlyB_tuned == 1);
...
%all A tuned by TS
A_tuned_ts_idx = find(tuned_logical.ts.Atuned == 1);
%all B tuned by TS
B_tuned_ts_idx = find(tuned_logical.ts.Btuned == 1);
...
Aonly_notTSb_idx = setdiff(Aonly_tuned_idx_si,B_tuned_ts_idx);

%only B tuned by SI, but also not TS A tuned
Bonly_notTSa_idx = setdiff(Bonly_tuned_idx_si,A_tuned_ts_idx);

%merge A and B chosen by exclusive
A_only_idx = union(Aonly_notSIb_idx, Aonly_notTSb_idx);
B_only_idx = union(Bonly_notSIa_idx,Bonly_notTSa_idx);
%end
...
%correct A
placeFieldEdges{1} = Place_cell{1}{1}.placeField.edge(input_idx_Aonly);
%correct B
placeFieldEdges{2} = Place_cell{1}{2}.placeField.edge(input_idx_Bonly);

%in A trials
idx_wo_placeFields{1} = input_idx_Aonly(find(cellfun(@isempty,placeFieldEdges{1}) ==1));
%in B trials
idx_wo_placeFields{2} = input_idx_Bonly(find(cellfun(@isempty,placeFieldEdges{2}) ==1));

%remove ROIs for A and B that do not have place fields
%for A trials
if ~isempty(idx_wo_placeFields{1})
    %copy
    Aonly_field_filtered = setdiff(input_idx_Aonly,idx_wo_placeFields{1});

else %keep the previous ROIs (copy only)
    Aonly_field_filtered = input_idx_Aonly;
end
%for B trials
if ~isempty(idx_wo_placeFields{2})
    %copy
    Bonly_field_filtered = setdiff(input_idx_Bonly,idx_wo_placeFields{2});

else %keep the previous ROIs (copy only)
    Bonly_field_filtered = input_idx_Bonly;
end
