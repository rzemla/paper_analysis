function [mean_TC,sem_TC,raw] = filter_convert_day_return_mean_sem_TC_global_neighbor(exp_struct,excl_day_combined_day_nan,exp_type,day_range,tuning_type)
...
%if exp_type == 2
    ...
    for aa=1:nb_animal
        %for A
        neighbor_TC_mean.A(aa,:) = diag(crossday_TC_main_diag_mean(aa).A,1);
        %for B
        neighbor_TC_mean.B(aa,:) = diag(crossday_TC_main_diag_mean(aa).B,1);
    end
...
day_TC_diag_mean_mean_exp.A = nanmean(neighbor_TC_mean.A,1);
day_TC_diag_mean_mean_exp.B = nanmean(neighbor_TC_mean.B,1);

%get sem across animals using correct count of animals per day
day_TC_diag_sem_mean_exp.A = nanstd(neighbor_TC_mean.A,0,1)./sqrt(sum(~isnan(neighbor_TC_mean.A),1));
day_TC_diag_sem_mean_exp.B = nanstd(neighbor_TC_mean.B,0,1)./sqrt(sum(~isnan(neighbor_TC_mean.B),1));
...
raw.animal.day_TC_diag_mean = neighbor_TC_mean;
