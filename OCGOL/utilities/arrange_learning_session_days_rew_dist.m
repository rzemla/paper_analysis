        ...
        case 9 %session 2 vs. 6; session 3 vs. 5
            for aa=1:6 %all animals
                ...
                if aa==1 || aa==2 || aa==3 || aa==5 || aa==6
                   ...
                   merge_theta_learn_days.B.rewB{aa,dd} = [];
                elseif aa==4
                     merge_theta_learn_days.A.rewA{aa,dd} = abs(diff(theta_reward_zone_learn{aa}.A.rewA{1,9},1,2));
                    merge_theta_learn_days.B.rewB{aa,dd} = abs(diff(theta_reward_zone_learn{aa}.B.rewB{1,9},1,2));
                end
            end
