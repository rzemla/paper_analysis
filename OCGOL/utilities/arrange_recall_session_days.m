...
for dd=2:9 %for day 1 vs 2 3 4... or equivalent day distance
    ...
    switch dd
        ...
        case 4 %session 4 vs. 7; session 3 vs. 4(use only first session
            for aa=1:5 %all animals
                %extract all the days that correspond to the time duration
                %BOTH
                ...
                merge_theta_recall_days.A{aa,dd} = theta_recall{aa}.A{3, 4};
                merge_theta_recall_days.B{aa,dd} = theta_recall{aa}.B{3, 4};
            end
        case 5 %session 2 vs. 6; session 3 vs. 5 (use only first session)
            for aa=1:5 %all animals
                %extract all the days that correspond to the time duration
                %BOTH
                ...
                %ONLY 1 equivalent session
                merge_theta_recall_days.A{aa,dd} = theta_recall{aa}.A{2, 6};
                merge_theta_recall_days.B{aa,dd} = theta_recall{aa}.B{2, 6};                
            end    
