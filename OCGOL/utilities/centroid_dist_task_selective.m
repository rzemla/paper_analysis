...
%for each trial
for tt=1:2
    [centroid_ct.all(tt,:),edges] = histcounts(max_bin_rate{1}{tt}, 0.5:1:100.5);
end
...
%% Plot histogram of max rate for each trial (all neurons)
% for all neurons
if 0
figure;
subplot(2,1,1)
...
%% Plot histogram selective for A or B only (selective neurons only)
