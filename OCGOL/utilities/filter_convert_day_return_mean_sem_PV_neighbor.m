function [mean_PV,sem_PV,raw] = filter_convert_day_return_mean_sem_PV_neighbor(exp_struct,excl_day_combined_day_nan,exp_type,day_range)
...
%if exp_type == 2
       ...
       for aa=1:nb_animal
           for dd1 = 1:size(day_range,2)
               for dd2 = 1:size(day_range,2)
                   %for A trials
                   crossday_compress_PV_mat.exp(aa).A{dd1,dd2} = crossday_PV_mat.exp(aa).A{day_range(dd1),day_range(dd2)};
                   %for B trials
                   crossday_compress_PV_mat.exp(aa).B{dd1,dd2} = crossday_PV_mat.exp(aa).B{day_range(dd1),day_range(dd2)};
               end
           end
       end
    
    %overwrite crossday_TC_mat with crossday_compress_TC_mat going forward
    crossday_PV_mat = crossday_compress_PV_mat;
    ...
    %extract means along the k+1 diagonal --> neighboring days correlation
    %means - insert means in a matrix: animal x neighbor correlations 
    for aa=1:nb_animal
        %for A
        neighbor_PV_mean.A(aa,:) = diag(crossday_PV_main_diag_mean(aa).A,1);
        %for B
        neighbor_PV_mean.B(aa,:) = diag(crossday_PV_main_diag_mean(aa).B,1);        
...
day_PV_diag_mean_mean_exp.A = nanmean(neighbor_PV_mean.A,1);
day_PV_diag_mean_mean_exp.B = nanmean(neighbor_PV_mean.B,1);

%get sem across animals using correct count of animals per day
day_PV_diag_sem_mean_exp.A = nanstd(neighbor_PV_mean.A,0,1)./sqrt(sum(~isnan(neighbor_PV_mean.A),1));
day_PV_diag_sem_mean_exp.B = nanstd(neighbor_PV_mean.B,0,1)./sqrt(sum(~isnan(neighbor_PV_mean.B),1));
...
raw.day_PV_diag_mean.A = neighbor_PV_mean.A;
raw.day_PV_diag_mean.B = neighbor_PV_mean.B;
