function [lap_id] = GOL_QC(Behavior,textures, rewards, trialType)
...
%blue = Odor A trials, late reward (high voltage trial signal)
     ...
     %% first cue should occur on each lap - not important for GOL
     ...
     for cc=1:size(trialType.time{rr},1)
         ...
         for ll=1:size(lap,2)
             ...
             if ~isempty(trial_check_temp)
                 %if B trial
                 if trialType.voltage{rr} < 3.5
                 trial_registered(ll) = 3;
                 %if A trial
                 elseif trialType.voltage{rr} > 3.5
