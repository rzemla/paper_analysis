...
if 0
    ...
    %% For grant added global remapper (McKnight - like)
    
    %adds a global remapper to the existing 3 types of neurons
    options.plotFigure2 = 1;
    raster_spiral_single_ses_grant(session_vars,CNMF_vars,removeROI,templates,options)
    
    %% Generate partial remapper display for supplement figure
    
    %partial_idx_by_animal_zone - not used, but can provide the indices
    %used to select the neurons
    partial_idx_by_animal_zone = 0;
    ...
    %% Generate global remapper display for supplement figure
    %this is a derivative of the function above
    
    %global_idx_by_animal_zone - not used, but can provide the indices
    %used to select the neurons
    %input the neurons selected from the remapper code
    global_idx_by_animal_zone = remapping_corr_idx.final.global;
    %bin distance separation for global remappers (multiply by cm
    %conversion factor to get cm separation)
    final_global_remap_bin_diff;
    
    options.plotFigure2 = 1;
    options.animalIdx = 11;
    %label to display the partial remapping schematic
    options.global = 1;
    raster_spiral_global_maps(session_vars,CNMF_vars,removeROI,templates,global_idx_by_animal_zone,final_global_remap_bin_diff,options)
